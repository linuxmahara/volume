...
uint256 hashGenesisBlock("0x00000e498d0973b9685f87140dd7f1814badd45459e728b187895d6025a25870");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Volume: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Volume Signed Message:\n";
...
{
    // Volume: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 50000); // Volume: 50k blocks halving
    //
    if (nSubsidy < COIN) nSubsidy = COIN;  // Minimum Number of Coin = 1
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 60;                                   // Volume: Difficulty adjusted every 10 mintues
static const int64 nTargetSpacing =   1 * 60;                                   // Volume: Every Minute
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Volume: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Volume: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "You know we will go skihigh right?";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043258110fa463ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1439328520;
        ...
        assert(block.hashMerkleRoot == uint256("0xad8a3c1b4bfb8aa985726d03001c739cf0199f5665a04c757f72719a9aba8ea7"));
...
unsigned char pchMessageStart[4] = { 0xd1, 0xd2, 0xd4, 0xdb }; // Volume: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VolumeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VolumeMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VolumeMiner(CWallet *pwallet)
{
    printf("VolumeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Volume-miner");
    ...
    try { loop {
        ...
        printf("Running VolumeMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VolumeMiner, pwallet));
}
